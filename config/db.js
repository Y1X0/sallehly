...
// [SCHED-FIX-BOOTRUN-01] راجع DECISIONS.md — الأربعة setInterval أعلاه
...
// قبل أول تنفيذ فعلي على الإطلاق. أي إعادة نشر/إعادة تشغيل بمعدّل أسرع من
// أقصر فاصل (ساعة واحدة — تنظيف التسجيلات المعلَّقة) — وهو الوضع الفعلي على
// منصّة نشر تُعيد بناء الحاوية بكل push — تعني أن النسخ الاحتياطي (٦ ساعات)
// قد لا يُنفَّذ **إطلاقاً** طوال عمر أي نسخة معيَّنة من العملية، رغم أن
// setInterval نفسه مسجَّل بشكل صحيح ولا خطأ ظاهر بالسجلات. الحل: تشغيل كل
// دالة مرة واحدة فوراً هنا أيضاً — بعد migrate(db) مباشرة (الجداول يجب أن
// توجد أولاً)، لا قبله — بالإضافة للفاصل الزمني المتكرر، لا بديلاً عنه.
// كل الدوال تمتص أخطاءها داخلياً بالفعل (try/catch)، فتشغيلها هنا لا يخاطر
// بإسقاط إقلاع السيرفر؛ الدالتان غير المتزامنتان (createDbBackup/
// cleanupOrphanUploads) بلا await هنا عمداً — نفس فلسفة استدعائهما داخل
