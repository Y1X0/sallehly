...
async function createRequest(request, token, { description = 'وصف تجريبي كافٍ للطول لإنشاء طلب جديد', service = SERVICE } = {}) {
  const res = await request.post('/api/requests', {
    headers: authHeader(token),
    multipart: { service, city: CITY, area: AREA, description },
  });
...
// [SEC-FIX-19] لم يكن هناك أي تحقق من تطابق خدمة الطلب مع خدمات الفني
// المسجَّلة عند تقديم العرض مباشرة (POST /requests/:id/offer) — التصفية
// كانت موجودة فقط بطرف القراءة (GET /requests)، فأي فني كان يقدر يرسل عرضاً
// على أي طلب مهما كانت خدمته عبر نداء API مباشر.
test.describe.serial('[SEC-FIX-19] منع تقديم عرض خارج خدمات الفني المسجَّلة', () => {
  let customer;
  let plumberTech;

  test.beforeAll(async ({ playwright }) => {
    const request = await playwright.request.newContext({ baseURL: 'http://127.0.0.1:4001' });
    customer = await registerAndVerify(request, { role: 'customer', extra: { name: 'عميل اختبار تطابق الخدمة', city: CITY } });
    plumberTech = await registerAndVerify(request, {
      role: 'technician',
      extra: {
        ...technicianRegisterExtra,
        name: 'فني سباكة - اختبار تطابق الخدمة',
        national_number: uniqueNationalNumber(),
        services: 'سباك,صيانة عامة', // لا يشمل "كهربائي" — SERVICE الثابت أعلاه
      },
      multipart: technicianAvatar,
    });
    await request.dispose();
  ...
  test('فني بخدمة مختلفة (سباك) لا يقدر يرسل عرضاً على طلب كهربائي', async ({ request }) => {
    const r = await createRequest(request, customer.token, {
      description: 'طلب كهربائي لاختبار رفض عرض فني بخدمة مختلفة',
    });

    const res = await request.post(`/api/requests/${r.id}/offer`, {
      headers: authHeader(plumberTech.token),
      form: { offer_price: '15', duration: 'خلال ساعة' },
    });
    ...
    // تأكيد إضافي: لم يُنشأ أي عرض فعلياً على الطلب.
    const offersRes = await request.get(`/api/requests/${r.id}/offers`, { headers: authHeader(customer.token) });
    expect((await offersRes.json()).offers.length).toBe(0);
  });

  test('نفس الفني يقدر يرسل عرضاً على طلب من خدمته الفعلية (سباك)', async ({ request }) => {
    const r = await createRequest(request, customer.token, {
      description: 'طلب سباكة لاختبار قبول عرض فني بنفس خدمته المسجَّلة',
      service: 'سباك',
    });
