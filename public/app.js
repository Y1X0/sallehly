...
let socket=null;
// [FEAT-DEDUP-01] راجع DECISIONS.md — عدد الفرص المجانية قبل خصم العمولة،
// كان الرقم "2" مكتوباً حرفياً بعدة مواضع عرض بهذا الملف. لا علاقة مباشرة
// بـFREE_TIER_QUOTA بمستودع الخادم (config/env.js) — ملف ثابت منفصل كلياً
// بلا نظام وحدات مشترك، فهذا ثابته الخاص لنفس القيمة.
    ...
    <div class="dash-card" style="margin-bottom:4px">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap">
        ...
        <div style="background:linear-gradient(135deg,rgba(124,58,237,.12),rgba(37,99,235,.08));border:1px solid rgba(124,58,237,.2);border-radius:20px;padding:16px 20px;text-align:center">
          ...
          <div style="font-size:26px;font-weight:900">${me.free_quota_used ?? (me.free_orders_used||0)}/${FREE_TIER_QUOTA}</div>
        </div>
      </div>
...
async function techDash(){let me=(await api('/api/me')).user;state.user=me;let menu=[['dash','الرئيسية'],['orders','الطلبات'],['balance','الرصيد والباقات'],['topups','طلبات الشحن'],['ledger','سجل الرصيد']];let c='';if(state.tab==='orders'){let j=await api('/api/requests');c=dashboardHero('لوحة الفني','تابع الطلبات القريبة وقدم عروضك بسرعة',[{label:'طلبات مناسبة',value:j.requests.length,up:'جديدة',icon:'🛒'},{label:'رصيدك',value:(me.balance||0)+' د.أ',up:'متاح',icon:'💳'},{label:'تقييمك',value:stars(me.rating_avg),up:'ثقة',icon:'⭐'},{label:'الأعمال',value:me.completed_jobs||0,up:'مكتملة',icon:'✅'}])+`<div class="dash-card"><h2>الطلبات المناسبة</h2>${reqTable(j.requests)}</div>`}else if(state.tab==='balance'){c=dashboardHero('الرصيد والباقات','اشحن رصيدك وتابع خصم عمولة الطلبات',[{label:'الرصيد',value:(me.balance||0)+' د.أ',up:'متاح',icon:'💳'},{label:'مجاني مستخدم',value:(me.free_quota_used ?? (me.free_orders_used||0))+'/'+FREE_TIER_QUOTA,up:'طلبات',icon:'🎁'},{label:'الباقات',value:state.meta.packages.length,up:'متاحة',icon:'📦'},{label:'الأعمال',value:me.completed_jobs||0,up:'منجزة',icon:'✅'}])+balancePage(me)}else if(state.tab==='topups'){let j=await api('/api/topups');c=dashboardHero('طلبات الشحن','تابع حالة دفعاتك وموافقات الإدارة',[{label:'طلبات الشحن',value:j.topups.length,up:'إجمالي',icon:'🚚'},{label:'الرصيد',value:(me.balance||0)+' د.أ',up:'متاح',icon:'💳'},{label:'باقات',value:state.meta.packages.length,up:'متوفرة',icon:'📦'},{label:'حالة الحساب',value:'فعال',up:'نشط',icon:'✅'}])+topupTable(j.topups)}else if(state.tab==='ledger'){let j=await api('/api/ledger');c=dashboardHero('سجل الرصيد','كل عمليات الخصم والشحن في مكان واحد',[{label:'عمليات',value:j.ledger.length,up:'مسجلة',icon:'📘'},{label:'الرصيد',value:(me.balance||0)+' د.أ',up:'حالي',icon:'💳'},{label:'طلبات',value:me.completed_jobs||0,up:'مكتملة',icon:'✅'},{label:'تقييم',value:stars(me.rating_avg),up:'فني',icon:'⭐'}])+ledgerTable(j.ledger)}else c=dashboardHero('لوحة الفني','إدارة احترافية لطلباتك ورصيدك وتقييمك',[{label:'الرصيد',value:(me.balance||0)+' د.أ',up:'متاح',icon:'💳'},{label:'طلبات مجانية',value:(me.free_quota_used ?? (me.free_orders_used||0))+'/'+FREE_TIER_QUOTA,up:'مستخدمة',icon:'🎁'},{label:'التقييم',value:stars(me.rating_avg),up:'ثقة',icon:'⭐'},{label:'الأعمال',value:me.completed_jobs||0,up:'مكتملة',icon:'✅'}])+`<div class="dash-grid"><div>${activityBox()}</div>${promoBox('زِد فرص قبولك','حدّث صورتك وخدماتك ومناطق عملك لتحصل على طلبات أكثر')}<div class="dash-card"><h2>ملخص سريع</h2><div class="mini-list"><div class="mini-list-row"><span>حالة الحساب</span><b>فعال</b></div><div class="mini-list-row"><span>العمولة لكل طلب</span><b>${me.active_commission!=null?me.active_commission+' د.أ':'—'}</b></div><div class="mini-list-row"><span>الأعمال المكتملة</span><b>${me.completed_jobs||0}</b></div></div></div></div>${chartsBox()}`;layout('لوحة الفني',menu,c)}
...
function techCard(t, service, city, area){
  ...
  const online=(Number(t.balance||0)>0 || Number(t.free_orders_used||0)<FREE_TIER_QUOTA);
...
function v22TechBalance(me){
  return dashboardHero('الرصيد والباقات','اشحن الرصيد من خلال باقة، ثم ارفع إثبات الدفع للإدارة.',[
    ...
    {label:'طلبات مجانية',value:(me.free_quota_used ?? (me.free_orders_used||0))+'/'+FREE_TIER_QUOTA,up:'مستخدمة',icon:'🎁'},
    {label:'الباقات',value:state.meta.packages.length,up:'متاحة',icon:'📦'},
...
setTimeout(renderBellBadge,200);
  ...
  // [FIX-RATE-03] + [FIX-RATE-LIMIT] دمج الحلّين: التحديث اللحظي الفعلي أصلاً صاير عبر socket.io
  // (v24BindRealtime)، وهاد الـ interval غرضه بس شبكة احتياطية (fallback) لو انقطع الاتصال اللحظي.
  // خفّفنا الفريكونسي من 3 ثواني إلى 8 ثواني (كافي جدًا لتحديث بادجات الإشعارات)، وأبقينا حماية
  // badgesInFlight المحلية حتى ما يبدأ نداء جديد قبل ما يخلص السابق — بالإضافة لحماية عامة مشابهة
  // موجودة أصلاً جوا v24RefreshBadges نفسها (بتحمي كمان من التكدس اللي سببه أحداث socket.io
  // المتزامنة زي new-request-created و offer-created و request-status-updated).
